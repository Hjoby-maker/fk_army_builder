...
class CSVParserService {
  dynamic parseCSVByFileName(String fileName, String csvString) {
    ...
    switch (fileName) {
      case 'Factions.csv':
        return parseFactionsFromCSV(csvString);
      case 'Abilities.csv':
        return parseAbilitiesFromCSV(csvString);
      case 'Datasheets.csv':
        return parseDatasheetsFromCSV(csvString);
      case 'Detachments.csv':
        return parseDetachmentsFromCSV(csvString);
      case 'Enhancements.csv':
        return parseEnhancementsFromCSV(csvString);
      case 'Source.csv':
        return parseSourcesFromCSV(csvString);
      case 'Stratagems.csv':
        return parseStratagemsFromCSV(csvString);
      case 'Detachment_abilities.csv':
        return parseDetachmentAbilitiesFromCSV(csvString);
      case 'Last_update.csv':
        return parseLastUpdateFromCSV(csvString);
      case 'Datasheets_abilities.csv':
        return parseDatasheetAbilitiesFromCSV(csvString);
      case 'Datasheets_detachment_abilities.csv':
        return parseDatasheetDetachmentAbilitiesFromCSV(csvString);
      case 'Datasheets_enhancements.csv':
        return parseDatasheetEnhancementsFromCSV(csvString);
      case 'Datasheets_keywords.csv':
        return parseDatasheetKeywordsFromCSV(csvString);
      case 'Datasheets_leader.csv':
        return parseDatasheetLeadersFromCSV(csvString);
      case 'Datasheets_models.csv':
        return parseDatasheetModelsFromCSV(csvString);
      case 'Datasheets_models_cost.csv':
        return parseDatasheetModelCostsFromCSV(csvString);
      case 'Datasheets_options.csv':
        return parseDatasheetOptionsFromCSV(csvString);
      case 'Datasheets_stratagems.csv':
        return parseDatasheetStratagemsFromCSV(csvString);
      case 'Datasheets_unit_composition.csv':
        return parseDatasheetUnitCompositionsFromCSV(csvString);
      case 'Datasheets_wargear.csv':
        return parseDatasheetWargearFromCSV(csvString);
      default:
        throw Exception('Парсер для файла $fileName не реализован');
    }
  }
  ...
  List<T> _parseCSV<T>(
    ...
    /*if (typeName == 'Detachment') {
      print(rows.length);
      print(rows[10].length);
      print(minColumns);
    }*/
  ...
  List<Detachment> parseDetachmentsFromCSV(String csvString) =>
      _parseCSV(csvString, Detachment.fromCSV, 5, 'Detachment');
  ...
  List<DatasheetDetachmentAbility> parseDatasheetDetachmentAbilitiesFromCSV(
          String csvString) =>
      _parseCSV(csvString, DatasheetDetachmentAbility.fromCSV, 2,
          'DatasheetDetachmentAbility');

  List<DatasheetEnhancement> parseDatasheetEnhancementsFromCSV(
          String csvString) =>
      _parseCSV(
          csvString, DatasheetEnhancement.fromCSV, 2, 'DatasheetEnhancement');
  ...
  List<DatasheetUnitComposition> parseDatasheetUnitCompositionsFromCSV(
          String csvString) =>
      _parseCSV(csvString, DatasheetUnitComposition.fromCSV, 3,
