...
class DatasheetKeywordDao extends DatabaseAccessor<AppDatabase> with _$DatasheetKeywordDaoMixin {
  DatasheetKeywordDao(AppDatabase db) : super(db);

  // CRUD
  Future<List<TdatasheetkeywordData>> getAllKeywords() => select(tdatasheetkeyword).get();
  
  Future<List<TdatasheetkeywordData>> getKeywordsByDatasheetId(int datasheetId) =>
      (select(tdatasheetkeyword)..where((t) => t.datasheetId.equals(datasheetId))).get();
  
  Future<void> insertAllKeywords(List<TdatasheetkeywordCompanion> keywordsList) =>
      batch((batch) {
        batch.insertAllOnConflictUpdate(tdatasheetkeyword, keywordsList);
      });
  
  Future<int> deleteKeywordsByDatasheet(int datasheetId) =>
      (delete(tdatasheetkeyword)..where((t) => t.datasheetId.equals(datasheetId))).go();
  ...
  Future<List<models.DatasheetKeyword>> getAllKeywordModels() async {
    ...
    return data.map((k) => models.DatasheetKeyword(
