...
class DetachmentAbilityDao extends DatabaseAccessor<AppDatabase> with _$DetachmentAbilityDaoMixin {
  DetachmentAbilityDao(AppDatabase db) : super(db);

  // CRUD
  Future<List<TdetachmentabilityData>> getAllDetachmentAbilities() => select(tdetachmentability).get();
  
  Future<TdetachmentabilityData?> getDetachmentAbilityById(int id) =>
      (select(tdetachmentability)..where((t) => t.id.equals(id))).getSingleOrNull();
  
  Future<List<TdetachmentabilityData>> getAbilitiesByDetachmentId(int detachmentId) =>
      (select(tdetachmentability)..where((t) => t.detachmentId.equals(detachmentId))).get();
  
  Future<int> insertDetachmentAbility(TdetachmentabilityCompanion ability) =>
      into(tdetachmentability).insert(ability);
  
  Future<void> insertAllDetachmentAbilities(List<TdetachmentabilityCompanion> abilitiesList) =>
      batch((batch) {
  ...
  // Поиск
  Future<List<TdetachmentabilityData>> searchDetachmentAbilities(String query) =>
  ...
  Future<List<models.DetachmentAbility>> getAllDetachmentAbilityModels() async {
    ...
    return data.map((a) => models.DetachmentAbility(
      id: a.id,
