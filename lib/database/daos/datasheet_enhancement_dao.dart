...
class DatasheetEnhancementDao extends DatabaseAccessor<AppDatabase> with _$DatasheetEnhancementDaoMixin {
  DatasheetEnhancementDao(AppDatabase db) : super(db);

  // CRUD
  Future<List<TdatasheetenhancementData>> getAll() => select(tdatasheetenhancement).get();
  
  Future<List<TdatasheetenhancementData>> getByDatasheetId(int datasheetId) =>
      (select(tdatasheetenhancement)..where((t) => t.datasheetId.equals(datasheetId))).get();
  
  Future<List<TdatasheetenhancementData>> getByEnhancementId(int enhancementId) =>
      (select(tdatasheetenhancement)..where((t) => t.enhancementId.equals(enhancementId))).get();
  ...
  Future<int> deleteByDatasheet(int datasheetId) =>
      (delete(tdatasheetenhancement)..where((t) => t.datasheetId.equals(datasheetId))).go();

  // Конвертация в модели
  Future<List<models.DatasheetEnhancement>> getAllModels() async {
    final data = await getAll();
    return data.map((e) => models.DatasheetEnhancement(
