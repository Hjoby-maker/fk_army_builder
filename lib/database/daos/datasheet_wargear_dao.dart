...
class DatasheetWargearDao extends DatabaseAccessor<AppDatabase> with _$DatasheetWargearDaoMixin {
  DatasheetWargearDao(AppDatabase db) : super(db);

  // CRUD
  Future<List<TdatasheetwargearData>> getAllWargear() => select(tdatasheetwargear).get();
  
  Future<List<TdatasheetwargearData>> getWargearById(int datasheetId, int lineInWargear) =>
      (select(tdatasheetwargear)
      ..where((t) => t.datasheetId.equals(datasheetId) & t.lineInWargear.equals(lineInWargear))
      ).get();
  ...
  Future<List<models.DatasheetWargear>> getAllWargearModels() async {
    ...
    return data.map((w) => models.DatasheetWargear(
