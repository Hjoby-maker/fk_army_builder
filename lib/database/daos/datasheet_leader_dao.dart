...
class DatasheetLeaderDao extends DatabaseAccessor<AppDatabase> with _$DatasheetLeaderDaoMixin {
  DatasheetLeaderDao(AppDatabase db) : super(db);

  // CRUD
  Future<List<TdatasheetleaderData>> getAllLeaders() => select(tdatasheetleader).get();
  
  Future<List<TdatasheetleaderData>> getLeadersByLeaderId(int leaderId) =>
      (select(tdatasheetleader)..where((t) => t.leaderId.equals(leaderId))).get();
  
  Future<List<TdatasheetleaderData>> getLeadersByAttachedId(int attachedId) =>
      (select(tdatasheetleader)..where((t) => t.attachedId.equals(attachedId))).get();
