...
class DatasheetModelCostDao extends DatabaseAccessor<AppDatabase> with _$DatasheetModelCostDaoMixin {
  DatasheetModelCostDao(AppDatabase db) : super(db);

  // CRUD
  Future<List<TdatasheetmodelcostData>> getAllCosts() => select(tdatasheetmodelcost).get();
  
  Future<TdatasheetmodelcostData?> getCostByIdAndLine(int datasheetId, int line) =>
  ...
  Future<int> deleteCostByIdAndLine(int datasheetId, int line) =>
      (delete(tdatasheetmodelcost)
      ..where((t) => t.datasheetId.equals(datasheetId) & t.line.equals(line))
      ).go();
  ...
  Future<List<models.DatasheetModelCost>> getAllCostModels() async {
    ...
    return data.map((c) => models.DatasheetModelCost(
