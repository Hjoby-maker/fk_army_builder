...
class DatasheetOptionDao extends DatabaseAccessor<AppDatabase> with _$DatasheetOptionDaoMixin {
  DatasheetOptionDao(AppDatabase db) : super(db);

  // CRUD
  Future<List<TdatasheetoptionData>> getAllOptions() => select(tdatasheetoption).get();
  
  Future<List<TdatasheetoptionData>> getOptionsByDatasheetIdAndLine(int datasheetId, int line) =>
  ...
  Future<int> deleteOptionsByDatasheet(int datasheetId, int line) =>
      (delete(tdatasheetoption)
      ..where((t) => t.datasheetId.equals(datasheetId) & t.line.equals(line))
      ).go();
  ...
  Future<List<models.DatasheetOption>> getAllOptionModels() async {
    ...
    return data.map((o) => models.DatasheetOption(
